...
```{r echo=FALSE}
ggplot(sector) + geom_point(aes(reorder(Institucion,Freq), Freq,  color=breaks), size=4) + theme_classic() + labs(y="% de fallecimientos dentro del grupo", x="Institución") +  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position="bottom") + labs( color="Casos totales")
```

### Descriptivos padecimientos
...
table<-rbind(obesidad,diabetes,hipertension,cardiovascular,renal,asma)
table<-as.data.frame(table)
table$Variable<-rep(c("Obesidad","Diabetes", "Hipertensión","Enf. Cardiovascular","Renal Crónica","Asma"), each=2)
table$Condicion<-rep(c("Sí","No"), by=6)
```

```{r echo=FALSE}
ggplot(table, aes(Percent, Variable, fill=Condicion)) + geom_bar(position="stack", stat="identity") + theme_classic() + labs(x="Porcentaje total de diagnosticados", fill="Condición")
```

### Descriptivos estado residencia
...
```{r echo=FALSE}
ggplot(edo) + geom_point(aes(reorder(Entidad,Percent), Percent), size=5) + theme_classic() + labs(y="% Casos diagnosticados totales", x="Entidad") +  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position="bottom")
```
