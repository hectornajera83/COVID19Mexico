...
ggplot(sexo, aes(x = x, y = predicted*100, colour=riesgo, group=riesgo)) +
  ...
  geom_pointrange(aes(ymin = conf.low*100, ymax = conf.high*100), fatten = 8, position=position_dodge(width=.3)) + labs(x="Sexo", y="Probabilidad fallecimiento (%). CI 95%", color = "Riesgo factores de salud") + 
   ylim(c(3,25)) + 
