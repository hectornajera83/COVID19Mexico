...
### Prevalencia (cruda) por padecimiento
...
```{r echo=FALSE}
ggplot(tab.m) + geom_point(aes(padecimiento, value*100, color=label), size=5) + 
  labs(y = "% de muertes dentro del grupo", color = "Condición de padecimiento") + theme_classic() + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x=element_blank(), legend.position="bottom") + scale_color_hue(l=40, c=55)
```

### Prevalencia (cruda) de muerte por sexo
...
### Prevalencia (cruda) de muerte por grupo de edad
...
### Prevalencia (cruda) de muerte por estado residencia
...
```{r echo=FALSE}
ggplot(edo) + geom_point(aes(reorder(D1.entidad_res,Freq), Freq), size=5) + theme_classic() + labs(y="% de muertes dentro del grupo", x="Entidad")
```


### Prevalencia (cruda) de muerte por condición indígena
...
La Prevalencia cruda es la proporción de muertes dentro de cada grupo poblacional. Por ejemplo, considere 100 mujeres y 100 hombres con una prevalencia cruda igual a 15% y 25%, respectivamente. Esto signfica que el 25% de los hombres con el padecimiento murió. 
